...
class ResourceController
{
	...
	*/
	...
	private function update_event($request)
	{
		$k_v = json_decode($request->getBody(), true);
		var_dump($k_v);
		$query = "UPDATE address, event SET cep='".$k_v["cep"]."', logradouro='".$k_v["logradouro"]."', bairro='".$k_v["bairro"]."', 	localidade='".$k_v["localidade"]."', uf='".$k_v["uf"]."', complemento='".$k_v["complemento"]."', numero='".$k_v["numero"]."', eventname='".$k_v["eventname"]."', eventtheme='".$k_v["eventtheme"]."', startdate='".$k_v["startdate"]."', enddate='".$k_v["enddate"]."', starttime='".$k_v["starttime"]."', codeventlocation='".$k_v["codeventlocation"]."', registrationfee='".$k_v["registrationfee"]."', subscriberslimit='".$k_v["subscriberslimit"]."', observation='".$k_v["observation"]."' WHERE event.codeventlocation=address.idaddress AND event.idevent='".$k_v["idevent"]."'";
	...
	{
		$k_v = json_decode($request->getBody(), true);
		$query = "UPDATE subscription SET coduser='".$k_v["coduser"]."', codevent='".$k_v["codevent"]."', subscriptiondate='".$k_v["subscriptiondate"]."', subscriptionstatus='".$k_v["subscriptionstatus"]."', sustatus='".$k_v["sustatus"]."';";
	...
	private function remove_subscription($request)
	{
		...
		$query = "UPDATE subscription SET subscriptionstatus=0 WHERE subscription.coduser='".$k_v["iduser"]."' AND subscription.codevent='".$k_v["codevent"]."';";
